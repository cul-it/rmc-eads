				...
				<div id="sidebar">
					...
					<xsl:choose>
						<xsl:when test="contains($publisher, 'Division')">
							<p id="sidebarlinks">
								<a href="https://rare.library.cornell.edu/about/contact"> Contact Rare &amp; Manuscript Collections </a>
							</p>
						</xsl:when>
						<xsl:otherwise>
							<p id="sidebarlinks">
		...
		<tr>
			...
			<td>
				<div>
					...
					<xsl:otherwise>
						<xsl:attribute name="STYLE">margin-left: <xsl:value-of select="$indent-value - 3"/>em; text-indent: -1em;</xsl:attribute>
						The digital preservation copy may be made available on <a href="https://rare.library.cornell.edu/about/contact">request</a> if a delivery copy is not already online.
					</xsl:otherwise>
				</xsl:choose>
